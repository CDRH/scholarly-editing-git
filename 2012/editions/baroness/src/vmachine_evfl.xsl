...
<xsl:stylesheet version="1.1" exclude-result-prefixes="tei"
   ...
   <xsl:variable name="indexPage">../main.baroness.html</xsl:variable>
   <xsl:variable name="help">../help.baroness.html</xsl:variable>
  ...
  "true" to "false" below -->
   ...
   <xsl:template name="headline">
      <div id="headline">
         ...
         <span class="versionCount">
            <xsl:text> has 33 witnesses</xsl:text>
            <!--  
   ...
   <xsl:template name="mainControls">
      <div id="mainControls">
         ...
         <input type="button" id="bibToggle" value="Witness List" onclick="toggleBiblio();" />
         &#8226;
     
   
         <input method="link" type="button" value="Help" onClick="parent.location='../help.baroness.html'"/>
         &#8226;
         
         <input method="link" type="button" value="Index" onClick="parent.location='../main.baroness.html'"/>
         &#xa0;
         &#xa0;
         &#xa0;
         &#xa0;
         ...
         &#xa0;
         &#xa0;
         <input type="button" id="newPanel" value="New Witness" class="special" onclick="openPanel();" />
         ...
         &#8226;-->
        <!-- to include line numbers as an option <label for="toggleLineNumbers">Line Numbers:</label>
      ...
      <div class="panel mssPanel">
         ...
         <div class="mssContent">
            <table style="table-layout:fixed; width:350px">
               <tr>
               <td>
                  <xsl:apply-templates select="//tei:body" /></td>
                  <td width="10%s" align="right" valign="top"> 
                     <xsl:for-each select="$witnesses">
                  ...
                  <xsl:for-each select="//tei:note[@type='image']/tei:witDetail[@target = concat('#',$witID)]//tei:graphic[@url]">
                     ...
                     <xsl:call-template name="imageLink">
                        ...
                        <xsl:with-param name="witTitle" select="ancestor::tei:witDetail" />
      ...
      <div class="panel" id="bibPanel">
         ...
         <div class="bibContent">
            ...
            <xsl:if test="tei:titleStmt/tei:editor">
               ...
               <xsl:for-each select="tei:titleStmt/tei:editor"><xsl:value-of select="." /> 
            ...
            <h4>Witness List</h4>
       <ul>
               <xsl:for-each select="tei:notesStmt/tei:note/tei:witDetail">
       <li>
                    <!-- 
                     <strong>
                        <xsl:text>Witness </xsl:text>
           ...
           <!-- <xsl:if test="tei:notesStmt/tei:note[@anchored = 'true' and not(@type='image' or @type='critIntro')]">
               <h4>Textual Notes</h4>
               <xsl:for-each select="tei:notesStmt/tei:note[@anchored = 'true' and not(@type='image' or @type='critIntro')]">
                  <div class="note">
                     <xsl:if test="@type">
                        <em class="label">
   ...
   <xsl:template match="tei:div1|tei:div2|tei:div3|tei:div4|tei:div5|tei:div6|tei:div7|tei:div8">
      <div>
         <xsl:attribute name="class">
   ...
   <xsl:template name="imageLink">
      ...
      <xsl:param name="increment" />
      <xsl:param name="witTitle" />
     
      <xsl:if test="$imageURL != ''">
         <xsl:if test="$witTitle != ''">
   ...
   <xsl:template match="tei:hi|tei:title|tei:emph">
      <span>
         <xsl:attribute name="class">
  ...
  <xsl:template match="tei:body//tei:witDetail//tei:emph">
      ...
      <i>
         ...
         <xsl:apply-templates />   <xsl:text>No title specified</xsl:text>
      </i>
   ...
   <xsl:template match="tei:milestone[@unit = 'stanza']">
      <div><xsl:attribute name="class"><xsl:text>stanzabreak</xsl:text>
            <xsl:if test="@ed">
               <xsl:text> </xsl:text>
  ...
  <!--added lg--> 
   <xsl:template match="tei:lg">
      ...
      </xsl:attribute> <xsl:apply-templates /></div><br/></xsl:template>
   ...
   <xsl:template match="tei:note">
    ...
    <xsl:template match="tei:body//tei:witDetail">
      <span class="noteicon">
         <xsl:if test="$notesFormat != 'popup'">
            <xsl:attribute name="style">
               <xsl:text>display: none;</xsl:text>
